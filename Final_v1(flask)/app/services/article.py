...
def search_person(person_name):
    """Searches for a person's name across all configured news websites and returns the results."""
    results = {}
    for site_name, site_config in news_sites.items():
        news_items = search_website(site_name, site_config, person_name)
        results[site_name] = news_items
    return results
